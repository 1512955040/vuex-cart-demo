  ...
  <div>
    ...
    >
      ...
      <el-table-column
        ...
        label="数量">
        <template v-slot="scope">
          <el-input-number v-model="scope.row.count" @change="updateProductValue({
            prodId:scope.row.id,
            count:$event
    ...
    <div>
      <p>已选 <span>{{checkCount}}</span> 件商品，总价：<span>{{totalPrice}}</span></p>
...
export default {
  ...
  computed:{
    ...
    ...mapGetters('cart',['checkCount','totalPrice']),
    checkedAll:{
      get(){
  ...
  methods:{
    ...mapMutations('cart',['updateAllProductChecked','updateProductChecked','updateProductValue','deleteFromCart'])
  },
  created(){
